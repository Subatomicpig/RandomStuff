...
int main()
{
	...
	testRender(base,iBase,city1,city2,city3,city4,city5,city6,middle_Silo,enemyMis);
	Cnecin();
	test = test;
	enemyMis.currentY += 40;
	testRender(base,iBase,city1,city2,city3,city4,city5,city6,middle_Silo,enemyMis);
	Cnecin();
	
	clicked = get_mouse_pos(&mouseX,&mouseY);
	plot_bitmap(iBase,mouseX,mouseY,missile_bitmap,4);
	Cnecin();
	*/
...
/*
	Purpose: Gets the current mouse position, and returns whether or not it was clicked
	Input  : an x and y coordinate to update via reference
	Returns: x and y mouse coordinates, and whether it was clicked or not
	Assume : nothing
*/
