...
#define VBASE_HIGH 0xFFFF8201
#define VBASE_MED  0xFFFF8203
...
char *get_video_base()
{
	...
	unsigned char *v_base_high = VBASE_HIGH;
	unsigned char *v_base_med  = VBASE_MED;
	char *v_base_start_combined;

	unsigned char v_med = *v_base_med;
	unsigned char v_high = *v_base_high;
	
	unsigned long v_base_start = (unsigned long)v_high << 16;
	v_base_start += (unsigned long)v_med << 8;
	
	v_base_start_combined = (char *)v_base_start;
	
	Super(old_ssp);
	return (v_base_start_combined);
}

void set_video_base_wrap(char *base)
{
	unsigned char *high = VBASE_HIGH;
	unsigned char *med = VBASE_MED;
	unsigned long frameStart = (long) base;
	unsigned char frameStartHigh;
	unsigned char frameStartMed = frameStart >> 8;
	long old_ssp;
	frameStart = frameStart >> 8;
	frameStartHigh = frameStart >> 8;
